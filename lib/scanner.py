import requests, time, sys, socket, hashlib, os
...
def monitor(overseer_config, config_section):
    while True:
        ...
        interval_time = overseer_config.config.get(config_section, 'interval_time')
...
def port_scan(host, domain, saved_ip):
    ...
    if target_ip != saved_ip:
        ...
        information_string = ("[+] Host: " + host + " translates to " + target_ip)
...
def html_diff(host, website_contents):
    ...
    if os.path.exists("log/" + host + "/scans/" + host + ".temp"):
        with open("log/" + host + "/scans/" + host + ".temp", "w") as current_html:
            current_html.write(str(soup))
            current_html.close()
    else:
        with open("log/" + host + "/scans/" + host + ".temp", "a") as current_html:
            current_html.write(str(soup))
            current_html.close()
    ...
    if os.path.exists("log/scans/" + host + ".html"):
        ...
        with open("log/" + host + "/scans/" + host + ".temp", "r") as current_html:
